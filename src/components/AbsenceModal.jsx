...
const AbsenceModal = ({ students, onClose, onSave }) => {
  ...
  // تحويل بيانات الغياب إلى {week, day}
  ...
  // "عرض" الأسبوع الحالي: رقم أسبوع أو "all"
  ...
  const getDateFromWeekAndDay = (week, day) => {
    const date = new Date(startDate);
    const daysToAdd = (week - 1) * 7 + day; // 0=الأحد
  ...
  // --- البحث العربي المصحح: يركز على الاسم الأول والأخير مع ترتيب النتائج ---
  ...
  const filteredStudents = useMemo(() => {
    ...
    // 1. تصفية الطلاب بناءً على الاسم الأول أو الأخير
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 z-50 flex justify-center items-center p-4">
      <div className="bg-gray-800 rounded-lg shadow-xl overflow-hidden max-w-6xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 overflow-y-auto flex-grow flex flex-col">
          ...
          {/* الجدول - نعيد تركيبه بالكامل عند تغيير العرض لمنع أي تداخل */}
          <div
                    ...
                    {weeksToDisplay.map(week =>
                      days.map(day => (
                        <td
                          ...
                          title={isStudentAbsent(student.id, week, day) ? getDateFromWeekAndDay(week, day) : ''}
                        >
                          {isStudentAbsent(student.id, week, day) ? getDateFromWeekAndDay(week, day) : ''}
                        </td>
                      ))
                    )}
                  </tr>
