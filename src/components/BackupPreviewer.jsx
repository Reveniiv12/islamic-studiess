...
const BackupPreviewer = ({ data, onConfirm, onCancel }) => {
    ...
    return (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-gray-950/98 backdrop-blur-3xl font-['Noto_Sans_Arabic',sans-serif]">
            <div className="bg-[#111827] border border-white/10 w-full max-w-7xl h-[96vh] rounded-[3rem] overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="flex-1 flex overflow-hidden">
                    {/* Sidebar Nav */}
                    <div className="w-64 bg-gray-950/50 border-l border-white/5 p-4 space-y-2 overflow-y-auto" dir="rtl">
                        {[
                            { id: 'summary', label: 'إحصائيات النسخة', icon: <FaFilter /> },
                            { id: 'students', label: 'قائمة الطلاب', icon: <FaUserGraduate /> },
                            { id: 'curriculum', label: 'المناهج والخطط', icon: <FaBook /> },
                            { id: 'messages', label: 'سجل المحادثات', icon: <FaComments /> },
                        ].map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`w-full flex items-center gap-3 px-4 py-4 rounded-2xl transition-all font-bold ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-gray-500 hover:bg-white/5 hover:text-gray-300'}`}
                            >
                    ...
                    <div className="flex-1 flex flex-col bg-[#111827]/40 overflow-hidden" dir="rtl">
                        ...
                        {activeTab !== 'summary' && (
                            <div className="p-4 border-b border-white/5 bg-[#1f2937]/10 flex flex-wrap gap-3 items-center">
                                <div className="flex items-center gap-3 bg-gray-900/80 border border-white/5 rounded-xl px-5 py-2 flex-1 min-w-[200px] shadow-inner">
                                ...
                                <select 
                                    className="bg-gray-800 text-white px-5 py-2 rounded-xl border border-white/5 outline-none text-xs font-bold"
                                    value={selectedGrade}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                            ...
                            {activeTab === 'messages' && (
                                <div className="flex h-full rounded-[2rem] overflow-hidden border border-white/5 bg-gray-900/50 shadow-2xl">
                                    {/* قائمة الطلاب الجانبية في الشات */}
                                    <div className="w-1/3 border-l border-white/5 bg-gray-950/30 flex flex-col">
                                    ...
                                    {/* منطقة الرسائل */}
                                    <div className="flex-1 flex flex-col bg-[#0f172a]/80 relative">
                                        {selectedChatStudent ? (
                                            <>
                                                {/* Header الشات */}
                                                <div className="p-4 bg-gray-800/60 border-b border-white/5 flex items-center gap-4">
                ...
                {viewingStudent && (
                    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 md:p-10 bg-black/95 backdrop-blur-3xl" dir="rtl">
                        <div className="bg-[#1a2332] border border-white/10 w-full max-w-6xl h-[94vh] rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col">
                            
                            {/* Header */}
                            <div className="p-8 bg-gray-900 border-b border-white/5 flex justify-between items-center">
                                <div className="flex items-center gap-6">
                                    <div className="relative">
                                        <img src={viewingStudent.photo || viewingStudent.image_url || '/images/1.webp'} alt="" className="w-28 h-28 rounded-[2rem] object-cover border-4 border-gray-700 shadow-2xl" />
                                        <div className="absolute -bottom-1 -right-1 w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-xl border-4 border-[#1a2332]"><FaCheck size={16} /></div>
                                    </div>
                                    <div className="text-white">
                                        <h3 className="text-3xl font-black mb-1 tracking-tight">{viewingStudent.name}</h3>
                                        <div className="flex gap-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                                            <span className="bg-gray-800 px-3 py-1 rounded-lg">السجل: {viewingStudent.nationalId || viewingStudent.national_id || '---'}</span>
                                            <span>{viewingStudent.grade_level || viewingStudent.grade} - {viewingStudent.section || viewingStudent.className}</span>
                                        </div>
                                    </div>
                                </div>
                                <button onClick={() => setViewingStudent(null)} className="p-4 bg-gray-800 hover:bg-red-500/20 text-white rounded-2xl transition border border-white/5"><FaTimes size={24} /></button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-[#0f172a]">
                                ...
                                {/* البطاقة الرقمية */}
                                <div className="bg-gray-800/40 p-6 rounded-[2.5rem] border border-white/5 mb-10 flex flex-col items-center justify-center gap-6">
                            ...
                            <div className="p-8 border-t border-white/5 bg-gray-900 flex justify-center">
                                <button onClick={() => setViewingStudent(null)} className="px-24 py-5 bg-blue-600 text-white rounded-3xl font-black hover:bg-blue-500 transition shadow-2xl shadow-blue-600/40 uppercase tracking-widest">إغلاق المستعرض</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Footer Main */}
                <div className="p-10 border-t border-white/5 bg-gray-950/80 flex justify-between items-center gap-10">
                    <div className="text-orange-400 bg-orange-400/5 px-10 py-6 rounded-[3rem] border border-orange-500/10 text-xs font-bold leading-relaxed flex-1">
                        تنبيه: يمكنك الآن استعراض المناهج والمحادثات مع فلاتر الصف والفصل. واجهة المحادثة مطابقة تماماً للموقع الأصلي مع دعم التواريخ الهجرية وتصنيفات المناهج.
                    </div>
                    <div className="flex gap-4">
                        <button onClick={onCancel} className="px-12 py-6 bg-gray-800 text-gray-300 rounded-[2rem] font-bold hover:bg-gray-700 transition">إلغاء</button>
                        <button onClick={onConfirm} className="px-16 py-6 bg-blue-600 text-white rounded-[2rem] font-black hover:bg-blue-500 transition shadow-2xl">تأكيد الاسترجاع</button>
                    </div>
                </div>
            </div>
        </div>
