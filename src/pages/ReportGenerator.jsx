...
const ReportGenerator = () => {
  ...
  // --- دوال الحفظ والالتقاط المحسنة ---

  const captureReportBlob = async () => {
  ...
  const handleSaveToPortfolio = async () => {
    ...
    try {
      ...
      if (!user) {
        alert('يجب تسجيل الدخول أولاً لحفظ التقرير');
        setIsSaving(false);
        return;
      }

      const blob = await captureReportBlob();
      if (!blob) throw new Error("فشل إنشاء صورة التقرير");
      ...
      alert('حدث خطأ أثناء عملية الحفظ: ' + error.message);
    } finally {
      setIsSaving(false);
    }
  };

  // --- دوال التعامل مع المدخلات (بدون تغيير) ---
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-['Noto_Sans_Arabic',sans-serif] pb-10" dir="rtl">
      ...
      <div className="max-w-[1600px] mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* قسم إدخال البيانات - تم الحفاظ على كافة الحقول */}
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-[#1e2530] w-full max-w-sm rounded-[2rem] p-8 border border-gray-700 shadow-2xl animate-in zoom-in duration-300">
            <div className="flex flex-col items-center text-center">
              ...
              <div className="flex flex-col w-full gap-3">
                <button onClick={() => navigate('/portfolio')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-blue-600/20">
                  <FaExternalLinkAlt className="text-sm" /> الانتقال إلى ملف الإنجاز
                </button>
                <button onClick={() => setShowSuccessModal(false)} className="w-full bg-[#28a745] hover:bg-[#218838] text-white font-bold py-3.5 rounded-2xl shadow-lg transition-all active:scale-95">
                  موافق
                </button>
              </div>
            </div>
