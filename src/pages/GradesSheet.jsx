...
const GradesSheet = ({
    ...
    updateStudentGrade // Keep this prop, although inputs are disabled here
}) => {
    ...
    // دالة لتوليد رؤوس الأعمدة الرقمية
    const renderHeaderNumbers = (count) => {
        return Array.from({ length: count }).map((_, i) => (
            <th key={`header_num_${i}`} scope="col"
                className="px-2 py-3 text-center text-xs font-medium text-gray-400 border-r border-gray-600 whitespace-nowrap">
                {i + 1}
            </th>
        ));
    };
    ...
    const handleExportToExcel = () => {
        ...
        // تم تعديل أسماء الأعمدة وحذف الأقواس (Max Scores)
        const headers = [
            'النجوم',
            'المجموع الكلي (100)',
            'أعمال السنة (40)', // جديد
            'التقييمات الرئيسية (60)', // جديد
            ...
            ...students[0].grades.performanceTasks.map((_, i) => `مهمة أدائية ${i + 1}`).reverse(),
            ...students[0].grades.participation.map((_, i) => `مشاركة ${i + 1}`).reverse(),
            ...students[0].grades.homework.map((_, i) => `واجب ${i + 1}`).reverse(),
            ...students[0].grades.classInteraction.map((_, i) => `تفاعل صفي ${i + 1}`).reverse(), // تم التعديل
            ...students[0].grades.tests.map((_, i) => `اختبار ${i + 1}`).reverse(),
            'الاسم'
        ];
        ...
        const studentData = students.map(student => {
            const rowData = [
                ...
                calculateCoursework(student.grades), // جديد
                calculateMajorAssessments(student.grades), // جديد
                ...
                ...student.grades.classInteraction.reverse(), // تم التعديل
    ...
    return (
        <div className="bg-gray-800 p-4 md:p-8 rounded-xl shadow-lg mt-4 md:mt-6 overflow-auto border border-gray-700 max-h-[80vh]">
            ...
            {/* تم إزالة خيارات حساب الاختبارات هنا */}
            <div className="flex gap-2 mb-4 text-sm justify-end">
                <button
                ...
                <table className="min-w-full divide-y divide-gray-700">
                    <thead className="bg-gray-700 sticky top-0 z-30">
                        <tr>
                            ...
                            {/* رؤوس الأعمدة الأساسية (الدرجات المحدثة) */}
                            <th scope="col" colSpan="2" className="text-center text-xs font-medium text-gray-400 uppercase tracking-wider border-l-2 border-r border-gray-500">الاختبارات (40)</th>
                            <th scope="col" colSpan="4" className="text-center text-xs font-medium text-gray-400 uppercase tracking-wider border-l border-r border-gray-600">التفاعل الصفي (10)</th>
                            ...
                            <th scope="col" colSpan="4" className="text-center text-xs font-medium text-gray-400 uppercase tracking-wider border-l-2 border-r border-gray-500">المهام الأدائية (10)</th>
                            ...
                            <th scope="col" colSpan="5" className="text-center text-xs font-medium text-gray-400 uppercase tracking-wider border-l-2 border-r border-gray-500">حفظ القرآن (10)</th>
                            <th scope="col" className="px-4 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider">المجموع (100)</th>
                        ...
                        <tr>
                            ...
                            {renderHeaderNumbers(4)} {/* التفاعل الصفي */}
                            {renderHeaderNumbers(10)}
                            {renderHeaderNumbers(10)}
                            {renderHeaderNumbers(4)} {/* المهام الأدائية */}
                            ...
                            <tr key={student.id} className="hover:bg-gray-700 transition-colors group">
                                ...
                                {student.grades.tests.slice(0, 2).map((grade, i) => (
                                    <td key={`test_${i}`} className="p-1 whitespace-nowrap text-sm text-center border-l-2 border-r border-gray-500">
                                        <input type="number" min="0" max="20" value={grade ?? ''} onChange={(e) => updateStudentGrade(student.id, "tests", i, e.target.value)}
                                            className="w-12 p-2 bg-gray-700 text-white text-center rounded focus:outline-none focus:ring-2 focus:ring-green-500" />
                                    </td>
                                ))}
                                {/* التفاعل الصفي (4 مربعات * 10) */}
                                {student.grades.classInteraction.slice(0, 4).map((grade, i) => (
                                    <td key={`ci_${i}`} className="p-1 whitespace-nowrap text-sm text-center border-r border-gray-600">
                                        <input type="number" min="0" max="10" value={grade ?? ''} onChange={(e) => updateStudentGrade(student.id, "classInteraction", i, e.target.value)}
                                            className="w-12 p-2 bg-gray-700 text-white text-center rounded focus:outline-none focus:ring-2 focus:ring-yellow-500" />
                                    </td>
                                ))}
                                {/* الواجبات (10 مربعات * 1) */}
                                ...
                                {student.grades.quranMemorization.slice(0, 5).map((grade, i) => (
                                    <td key={`qm_${i}`} className={`p-1 whitespace-nowrap text-sm text-center ${i === 0 ? 'border-l-2 border-gray-500' : ''} border-r border-gray-600`}>
                                        <input type="number" min="0" max="10" value={grade ?? ''} onChange={(e) => updateStudentGrade(student.id, "quranMemorization", i, e.target.value)}
                                            className="w-12 p-2 bg-gray-700 text-white text-center rounded focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                    </td>
                                ))}
