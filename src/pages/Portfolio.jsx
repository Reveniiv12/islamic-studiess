...
const Portfolio = () => {
  ...
  useEffect(() => {
    onAuthStateChanged(auth, (user) => {
      if (user) {
        const publicUrl = `${window.location.origin}/portfolio/${user.uid}`;
        setQrCodeData(publicUrl);
        const savedFiles = JSON.parse(localStorage.getItem(`portfolioFiles-${user.uid}`)) || [];
        setFiles(savedFiles);
        const savedInfo = JSON.parse(localStorage.getItem(`portfolioTeacherInfo-${user.uid}`)) || {};
  ...
  const saveFilesToLocalStorage = (newFiles) => {
    const user = auth.currentUser;
    if (user) {
      localStorage.setItem(`portfolioFiles-${user.uid}`, JSON.stringify(newFiles));
    }
  };
  ...
  const saveTeacherInfo = () => {
    const user = auth.currentUser;
    if (user) {
      localStorage.setItem(`portfolioTeacherInfo-${user.uid}`, JSON.stringify(teacherInfo));
    }
